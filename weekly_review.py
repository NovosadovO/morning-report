    ...
    try:
        ...
        return {"ok": True, "last": last_v, "diff": diff,
                "text": (f"{last_v} кг (замір {last_d}), за тиждень {diff:+} кг, "
                         f"до цілі 75 кг залишилось {round(last_v - 75, 1)} кг")}
    except Exception as e:
...
цілі: фінансова незалежність, вага 75 кг, робота в інвестиціях, регулярний біг).
...
def _local_review(d: dict, period: str) -> dict:
    ...
    if w.get("ok"):
        goals.append({"title": f"вага {round(w['last'] - 0.5, 1)} кг",
                      "why": f"зараз {w['last']} кг, ціль 75 кг"})
    h = d["habits"]
    if h.get("ok"):
