...
_STYLE = (
    ...
    "06:00-18:00 / 18:00-06:00, ціль — 75 кг). Пиши українською, тепло, "
    "по-людськи, конкретно. Використовуй ТІЛЬКИ факти з блоку даних. "
    "ЗАБОРОНЕНО вигадувати числа чи стан (настрій/енергію/стрес), яких немає в "
    "даних. Якщо якогось показника немає — так і скажи і попроси надіслати, а "
    "не вигадуй за нього. Без води і банальностей.\n"
    "ОБСЯГ І ГЛИБИНА (прямий запит Олега 28.08 — він хоче БІЛЬШЕ опису): "
    "пиши розгорнуто, 400-700 слів. Кожен показник не просто називай, а "
    "РОЗБИРАЙ: що це число означає для його тіла, як воно змінилось за тиждень "
    "і місяць (тренд у цифрах), чому так вийшло (зв'яжи зі зміною, сном, "
    "рухом), до чого веде, якщо так триватиме, і що конкретно зробити сьогодні. "
    "Показуй зв'язки між метриками — не тільки сон↔кроки↔вага↔пульс, а й "
    "графік зміни↔якість/стабільність сну, і якщо в даних є body battery чи "
    "стрес з годинника — енергія/стрес↔сон↔рух. Один показник окремо мало "
    "значить, головне — зв'язки. Додай прогноз по цифрах і одну річ, яку він "
    "сам не помітив.\n"
    "ПИТАННЯ (запит Олега 20.09 — 'мало видно коуча', хоче більше запитань і "
    "пропозицій, а не тільки констатацію фактів): якщо в даних є прогалина, "
    "суперечність, чи щось справді варте уточнення для точнішого аналізу — "
    "поставь Олегу коротке пряме питання (не загальне 'як справи', а конкретне "
    "по даних). Не в кожному звіті обов'язково, лише коли це реально доречно "
    "з контексту — ти сам вирішуєш, коли питання додає цінність, а коли ні."
)


def _ai(prompt: str, tokens: int = 1800) -> str:
    try:
...
def morning_plan(send: bool = True) -> str:
    ...
    prompt = (_STYLE + "\n\nДАНІ:\n" + "\n".join(ctx) + "\n\n"
              ...
              "🍽 ЇЖА — що і о котрій, під ціль 75 кг\n"
              "🏃 РУХ — конкретне вікно для руху в цьому графіку\n"
              "🎯 ФОКУС ДНЯ — одна річ, головна на сьогодні\n"
              "Якщо доречно (є прогалина в даних чи щось варте уточнення) — "
              "додай ще один короткий блок ❓ ПИТАННЯ з конкретним запитанням; "
              "якщо недоречно — просто не додавай його, це необов'язково.\n"
              "Наприкінці один рядок мотивації. Без вступу.")
    body = _ai(prompt, 2000)
    if not body:
...
def evening_review(send: bool = True) -> str:
    ...
    prompt = (_STYLE + "\n\nДАНІ:\n" + "\n".join(ctx) + "\n\n"
              ...
              "3) Конкретні дії на завтра під завтрашню зміну — стільки, скільки "
              "реально випливає з цих даних (не обмежуй трьома, якщо приводів "
              "більше — по вазі, сну, крокам, пульсу, енергії/стресу)\n"
              "4) Один рядок про сон: о котрій лягати сьогодні\n"
              "5) Якщо доречно — коротке уточнююче питання про те, чого в "
              "даних сьогодні не хватає для точнішого розбору (необов'язково)\n"
              "Коротко, без вступу, без повтору самих чисел рядок за рядком.")
    body = _ai(prompt, 1300) or "AI зараз недоступний — нижче тільки цифри."
...
def sleep_report(send: bool = True) -> str:
    ...
    ctx = ["СОН: останній " + str(s.get("last") or "—") + " год за " + str(s.get("last_day") or "—")
           + (f" (!! {s.get('days_ago')} дн. тому, НЕ сьогодні !!)" if (s.get("days_ago") or 0) >= 1 else " (сьогодні)")
           + " | сер.7д " + str(s.get("avg7") or "—") + " | сер.30д " + str(s.get("avg") or "—")
           + " | тренд " + str(s.get("trend") or "—"),
           "Ціль сну: " + str(SLEEP_GOAL_H) + " год",
           "Історія (останні дні):\n" + hist,
           "Зміни на 7 днів вперед:\n" + ("\n".join(shifts) if shifts else "—")]
    prompt = (_STYLE + "\n\nДАНІ:\n" + "\n".join(ctx) + "\n\n"
              "Якщо позначено '!! N дн. тому, НЕ сьогодні !!' — сьогоднішній сон ще НЕ "
              "приходив, це старе значення. ЗАБОРОНЕНО писати 'ти спав сьогодні' чи "
              "називати сьогоднішню дату для нього — прямо скажи, що дані застаріли на "
              "N днів і попроси свіжі.\n"
              ...
              "5) Конкретні правила саме для Олега — стільки, скільки виправдано "
              "цими даними (мінімум 3, більше якщо є ще приводи)\n"
...
def chart(days: int = 30) -> bytes:
    ...
    specs = [
        (ax[0][0], weight, "⚖️ Вага, кг", "#e63946", WEIGHT_GOAL, "ціль 75"),
...
def _period_report(days: int, title: str, tag: str, send: bool) -> str:
    ...
    prompt = (_STYLE + "\n\nДАНІ ЗА " + str(days) + " ДНІВ:\n" + facts + "\n\n"
              ...
              "3) Що саме заважає дійти до 75 кг — конкретно\n"
              "4) План на наступний період: стільки пунктів з числами, скільки "
              "реально випливає з даних (не менше 3, без штучної межі)\n"
              "5) Одне питання до Олега наприкінці")
    body = _ai(prompt, 2400) or "AI недоступний — нижче факти."
...
def full_report(send: bool = True) -> str:
    ...
    # прогноз досягнення 75 кг за фактичним темпом
    ...
    prompt = (_STYLE + "\n\nПОВНІ ДАНІ ТРЕКІНГУ:\n" + facts + "\n\n"
              ...
              "5) 🎯 РЕКОМЕНДАЦІЇ — стільки конкретних дій з числами і термінами, "
              "скільки реально випливає з даних вище (не обмежуй п'ятьма, якщо "
              "приводів по вазі/сну/крокам/пульсу/енергії/стресу/зміні більше)\n"
              "6) 🕳 ДІРИ В ДАНИХ — чого не хватає для точнішого аналізу\n"
              "Пиши предметно, з числами з даних. Без загальних порад.")
    body = _ai(prompt, 2200) or "AI недоступний — нижче лише факти."
