    ...
    if gemini_key:
        ...
        for _attempt_b in range(3):  # 3 спроби
            try:
                ...
                _full_report_ctx = "\n\n".join(_all_report_text_parts)[:4000]

                _brief_prompt = (
                    f"Ти — персональний тренер, асистент і помічник Олега Новосадова (Кошіце, Словаччина).\n"
                    f"Зараз {now_local.strftime('%H:%M')}, {now_local.strftime('%d.%m.%Y')}.\n"
                    f"{shift_hint}\n\n"
                    f"=== ПОВНИЙ ЗВІТ (ВСЕ ЩО СЛІДКУЄ БОТ) ===\n{_full_report_ctx}\n===================\n\n"
                    f"Прочитай ВЕСЬ звіт і напиши живий персональний брифінг — 12-15 речень.\n"
                    f"Починай з 'Олег,'. Без вступів, без 'Ось', без 'Звичайно', без шаблонних фраз.\n\n"
                    f"СТРУКТУРА (кожен пункт — 1-2 речення, природно, без заголовків):\n"
                    f"1. Поточний момент — що зараз відбувається (зміна/вільний час/час доби)\n"
                    f"2. Здоров'я — вага, тенденція (ростемо/падаємо), HRV якщо є\n"
                    f"3. Активність — чи були кроки/біг сьогодні, оціни рівень\n"
                    f"4. Звички — скільки виконано, що пропущено, похвали або м'який докір\n"
                    f"5. Ліки Armolopid — чи прийняв, нагадай якщо ні\n"
                    f"6. Крипто/фінанси — головні рухи BTC/ETH/AVAX/ONDO, портфель\n"
                    f"7. Погода та план на вулицю — що варто зробити сьогодні\n"
                    f"8. Календар — найближчі події, що не пропустити\n"
                    f"9. Пошта — є щось важливе від Мароша або інше\n"
                    f"10. Головна порада як тренер — 1 конкретна дія яку ЗАРАЗ зробити для цілей (схуднення, фінанси, нова робота)\n"
                    f"11. Мотивація — коротка, щира, без банальностей\n\n"
                    f"ВАЖЛИВО: використовуй ТІЛЬКИ реальні дані зі звіту (числа, ціни, імена, час).\n"
                    f"Якщо даних немає — пропусти той пункт. Тон: як близький друг-тренер, прямо і по-людськи.\n"
                    f"ЯКЩО зміна вже йде — 'ти зараз на зміні', не 'чекає'. [seed:{_seed_b}]"
                )
