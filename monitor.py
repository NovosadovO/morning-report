...
def _get_astro_ai_analysis(astro_text: str, gemini_key: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"{astro_text[:4000]}\n"
            f"================================\n\n"
            f"Напиши стислий астро-аналіз дня. Структура ТОЧНО така:\n\n"
            f"🌙 МІСЯЦЬ\n"
            ...
            f"🏃 ТІЛО/БІГ\n"
            f"[2 речення: астро-стан для фізичної активності + чи варто бігти сьогодні]\n\n"
            f"📌 ПОРАДА ДНЯ\n"
            f"[1-2 речення: головне що зробити або чого уникати сьогодні виходячи з аспектів]\n\n"
            f"ПРАВИЛА: тільки планети/аспекти з звіту. БЕЗ вступів і прощань. Конкретно і коротко. Мова: українська."
        )
        body = json.dumps({
            "contents": [{"parts": [{"text": prompt}]}],
        ...
        for _attempt_b in range(3):  # 3 спроби
            try:
                ...
                _brief_prompt = (
                    ...
                    f"⚡ ЗАРАЗ\n"
                    f"[2-3 речення: що відбувається зараз (зміна/вільний час), головний пріоритет на найближчу годину]\n\n"
                    f"💹 КРИПТО\n"
