...
def _get_astro_ai_analysis(astro_text: str, gemini_key: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"{astro_text[:3000]}\n"
            f"================================\n\n"
            f"На основі цього астро-звіту напиши ТІЛЬКИ астрологічний аналіз — 15-20 речень.\n"
            f"ОБОВ'ЯЗКОВО охопи:\n"
            f"• Місяць сьогодні: знак, фаза, як це впливає на енергію і настрій Олега\n"
            f"• Основні транзити планет: які активні зараз, що означають конкретно для Олега\n"
            f"• Фінанси та крипто через астро: чи сприятливий день для угод, покупок, рішень\n"
            f"• Здоров'я та фізична активність: чи підходить день для бігу, навантаження\n"
            f"• Стосунки та спілкування: що каже астро про взаємодію з людьми сьогодні\n"
            f"• 1 конкретна астро-порада на день: що варто зробити / чого уникати\n\n"
            f"Пиши суцільним текстом без заголовків. Мова: українська. Тон: конкретний і практичний.\n"
            f"Тільки реальні дані з астро-звіту — без вигадки."
        )
        body = json.dumps({
            "contents": [{"parts": [{"text": prompt}]}],
    ...
    if gemini_key:
        ...
        for _attempt_b in range(3):  # 3 спроби
            try:
                ...
                _brief_prompt = (
                    f"Ти — персональний тренер, асистент і нутриціолог Олега Новосадова (Кошіце, Словаччина).\n"
                    f"Зараз {now_local.strftime('%H:%M')}, {now_local.strftime('%d.%m.%Y')}.\n"
                    f"{shift_hint}\n\n"
                    ...
                    f"БЕЗ будь-яких астрологічних коментарів — астро є окремим блоком у звіті.\n\n"
