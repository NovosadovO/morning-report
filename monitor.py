...
def _get_astro_ai_analysis(astro_text: str, gemini_key: str, shift_hint: str = "") -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"СТАТУС: {shift_hint}\n\n"
            f"=== ПОВНИЙ АСТРОЛОГІЧНИЙ ЗВІТ ===\n"
            f"{_full_astro[:6000]}\n"
            f"================================\n\n"
            f"Напиши ПОВНИЙ детальний астро-аналіз дня для Олега. Структура ТОЧНО така:\n\n"
            f"🌙 МІСЯЦЬ\n"
            f"[Знак місяця + фаза + 3-4 речення: яка загальна енергія дня, що сприятливо, що ні, чому]\n\n"
            f"🪐 ТРАНЗИТИ ПЛАНЕТ\n"
            f"[Для КОЖНОГО транзиту/переходу планети з звіту:\n"
            f"• Назва: «Планета у Знаку» або «Планета входить в Знак»\n"
            f"• Що це означає загалом (2-3 речення)\n"
            f"• Як конкретно впливає на Олега — робота/завод, фінанси/крипто, тіло/спорт, відносини (3-4 речення)\n"
            f"• Рекомендація: що робити / чого уникати]\n\n"
            f"⚡ АСПЕКТИ\n"
            f"[Для КОЖНОГО аспекту з звіту окремий блок:\n"
            f"• Назва: «Планета1 аспект Планета2»\n"
            f"• Тип аспекту: напружений (квадрат/оппозиція) або гармонійний (трин/секстиль) або нейтральний (квінконкс/соєднання)\n"
            f"• Що це означає астрологічно — загальне значення (2 речення)\n"
            f"• Як це впливає на Олега КОНКРЕТНО сьогодні: фінанси, крипто, тіло, рішення, енергія, робота (3-4 речення)\n"
            f"• Конкретна порада або попередження]\n\n"
            f"💰 ФІНАНСИ/КРИПТО\n"
            f"[3-4 речення: синтез всіх аспектів — чи сприятливо для BTC/ETH/AVAX/ONDO сьогодні. "
            f"Конкретна порада: купувати/тримати/фіксувати прибуток. Чому саме так виходячи з планет.]\n\n"
            f"🏃 ТІЛО/БІГ\n"
            f"[3 речення: астро-стан для фізичної активності. Чи варто бігти/тренуватись сьогодні. "
            f"Де зберігати енергію якщо аспекти напружені.]\n\n"
            f"⚡ РОБОТА/ЗАВОД\n"
            f"[2-3 речення: як аспекти впливають на робочу зміну Олега на Minebea Mitsumi. "
            f"Концентрація, безпека, взаємодія з колегами — що варто мати на увазі.]\n\n"
            f"📌 ПОРАДА ДНЯ\n"
            f"[2-3 речення: найголовніше що зробити і чого уникати сьогодні виходячи з ВСІХ аспектів і транзитів. "
            f"Конкретно для Олега, без кліше.]\n\n"
            f"ПРАВИЛА:\n"
            f"- Перелічи ВСІ аспекти і транзити з звіту — не пропускай жодного\n"
            f"- Тільки планети/аспекти з звіту — не додавай вигаданих\n"
            f"- БЕЗ вступів і прощань\n"
            f"- Конкретно і змістовно — кожен аспект по 3-5 речень\n"
            f"- Мова: українська"
        )
        body = json.dumps({
            "contents": [{"parts": [{"text": prompt}]}],
