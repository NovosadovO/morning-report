    ...
    if gemini_key:
        try:
            ...
            ai_prompt = (
                ...
                f"Відповідай ТІЛЬКИ 1-2 реченнями українською без вступу, без 'Звичайно', без 'Привіт'. "
                f"Конкретна порада для Олега на основі його реального розкладу і даних. [seed:{seed}]"
            )
    ...
    if gemini_key:
        try:
            ...
            _brief_prompt = (
                ...
                f"{weight_hint} "
                f"Відповідай ТІЛЬКИ одним реченням українською без вступу, без 'Звичайно', без 'Ось'. "
                f"Речення починається зі слова 'Олег,' і закінчується крапкою. "
                f"Приклад: 'Олег, сьогодні рання зміна і тепло — добрий день для пробіжки після роботи.' [seed:{_seed_b}]"
            )
