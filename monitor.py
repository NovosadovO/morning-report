...
def _get_astro_ai_analysis(astro_text: str, gemini_key: str) -> str:
    """
    ...
    try:
        ...
        prompt = (
            ...
            f"{astro_text[:4000]}\n"
            f"================================\n\n"
            f"На основі цього астро-звіту напиши детальний аналіз — РІВНО 30 речень.\n"
            f"Структура (пиши суцільним текстом, БЕЗ заголовків):\n\n"
            f"МІСЯЦЬ (5 речень): Точний знак і фаза Місяця. Як ця фаза впливає на настрій і рішення сьогодні. Чи сприятливий час для нових справ чи краще завершувати старі. Енергетика дня — активність чи відпочинок. Найкращий час доби для важливих рішень.\n\n"
            f"АСПЕКТИ (10 речень): Для КОЖНОГО активного аспекту з звіту — назва планет, тип аспекту, точний вплив на Олега. Що цей аспект означає конкретно для нього прямо зараз. Як довго він активний. Наприклад: 'Марс сексильний Юпітер підсилює фізичну витривалість — ідеальний час для тренування'.\n\n"
            f"ФІНАНСИ ТА КРИПТО (4 речення): Які планети впливають на фінансові рішення сьогодні. Чи сприятливий день для покупки/продажу крипто (BTC/ETH/AVAX/ONDO). Конкретна рекомендація — діяти чи чекати і чому. Ризики якщо є.\n\n"
            f"ЗДОРОВ'Я ТА БІГ (4 речення): Астро-стан фізичної активності сьогодні. Чи підходить для бігу і тренування. Частини тіла під увагою сьогодні. Рекомендація по харчуванню або відпочинку.\n\n"
            f"РОБОТА ТА РОЗВИТОК (4 речення): Сприятливість для навчання інвестиціям. Чи хороший час для контактів і переговорів. Продуктивність і концентрація.\n\n"
            f"ГОЛОВНА ПОРАДА (3 речення): 1 конкретна дія яку треба зробити сьогодні виходячи з аспектів. Чого уникати. Ключове слово дня.\n\n"
            f"ПРАВИЛА: тільки реальні планети/аспекти з звіту. БЕЗ вигадки. Тон: конкретний астролог-практик. Мова: українська."
        )
        body = json.dumps({
            "contents": [{"parts": [{"text": prompt}]}],
...
def main():
    ...
    if astro_text:
        ...
        # Блок 6б: Окремий AI астро-аналіз — одразу після астро
        _astro_ai = _get_astro_ai_analysis(astro_text, gemini_key)
        if _astro_ai:
            parts.append(f"🔮🤖 <b>АСТРО-АНАЛІЗ</b>\n<i>{esc(_astro_ai)}</i>")
        ...
        for _attempt_b in range(3):  # 3 спроби
            try:
                ...
                _brief_prompt = (
                    ...
                    f"{_full_report_ctx}\n"
                    f"=================\n\n"
                    f"Напиши структурований персональний брифінг — РІВНО 35 речень.\n"
                    f"Формат — блоки з емодзі заголовками, 5 блоків по 7 речень:\n\n"
                    f"⚡ ЗАРАЗ\n"
                    f"Що відбувається прямо зараз (зміна/вільний час). Головне на найближчу годину. Конкретна дія прямо зараз.\n\n"
                    f"💹 КРИПТО\n"
                    f"BTC/ETH/AVAX/ONDO — точні ціни і % рух. Стан ринку. Тримати/докупити/зафіксувати + чому.\n\n"
                    f"⚖️ ТІЛО\n"
                    f"Вага + відстань до цілі 79 кг. Кроки/біг. Звички — скільки з 6. Armolopid прийнято?\n\n"
                    f"🌤 ДЕНЬ\n"
                    f"Погода Кошіце — температура + чи варто виходити. Важливі події з календаря. Критична пошта?\n\n"
                    f"🎯 ЦІЛЬ\n"
                    f"1 конкретна дія сьогодні (схуднення АБО крипто АБО нова робота). Прямо і без кліше.\n\n"
                    f"ПРАВИЛА: тільки реальні числа зі звіту. Якщо даних немає — пропусти блок. БЕЗ астро. БЕЗ вступів. Тон: прямий тренер. [seed:{_seed_b}]"
                )
