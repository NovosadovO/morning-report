...
"""
    ...
    try:
        ...
        prompt = (
            ...
            f"СТАТУС: {shift_hint}\n\n"
            f"{NATAL_CHART}\n"
            f"=== СЬОГОДНІШНІЙ АСТРОЛОГІЧНИЙ ЗВІТ (транзити) ===\n"
            f"{_full_astro[:5000]}\n"
            f"================================\n\n"
            f"Напиши ПОВНИЙ персональний астро-аналіз дня для Олега з урахуванням його натальної карти.\n"
            f"Структура ТОЧНО така:\n\n"
            f"🌙 МІСЯЦЬ\n"
            f"[Знак і фаза транзитного місяця. Як він взаємодіє з натальним Місяцем Олега (Близнюки, 11-й дім). "
            f"3-4 речення: загальна емоційна енергія дня, що посилює/послаблює, рекомендація.]\n\n"
            f"🪐 ТРАНЗИТИ ПЛАНЕТ\n"
            f"[Для КОЖНОЇ транзитної планети зі звіту — окремий блок:\n"
            f"Назва: «♄ Сатурн у Близнюках — 11-й натальний дім» (транзитний знак + який натальний дім активує)\n"
            f"• Загальне значення транзиту (1-2 речення)\n"
            f"• Персональний вплив на Олега через його натальну карту — яку натальну планету або дім торкається, "
            f"що активує (3-4 речення: фінанси, крипто, робота/завод, тіло, відносини, цілі)\n"
            f"• Рекомендація або попередження]\n\n"
            f"⚡ АСПЕКТИ ДО НАТАЛЬНОЇ КАРТИ\n"
            f"[Для КОЖНОГО аспекту зі звіту — окремий блок:\n"
            f"Назва: «☿ Транзитний Меркурій △ натальний Марс (Терези, 3-й дім)»\n"
            f"• Тип: напружений / гармонійний / нейтральний\n"
            f"• Астрологічне значення цього аспекту (2 речення)\n"
            f"• Конкретний вплив на Олега через натальні позиції: що саме активується в його карті, "
            f"як це проявиться сьогодні (3-4 речення)\n"
            f"• Порада: що робити / чого уникати]\n\n"
            f"💰 ФІНАНСИ/КРИПТО (BTC/ETH/AVAX/ONDO)\n"
            f"[Синтез: які транзити і аспекти впливають на 2-й і 8-й натальні доми Олега (гроші/інвестиції). "
            f"Конкретна порада: купувати/тримати/фіксувати. 3-4 речення.]\n\n"
            f"🏃 ТІЛО/БІГ/ЗДОРОВ'Я\n"
            f"[Вплив транзитів на 6-й дім Олега (Сатурн+Уран+Нептун натально). "
            f"Чи варто бігти/тренуватись сьогодні. 3 речення.]\n\n"
            f"🏭 РОБОТА/ЗАВОД (Minebea Mitsumi)\n"
            f"[Як транзити впливають на 6-й та 10-й доми. Концентрація, безпека, колеги. 2-3 речення.]\n\n"
            f"📌 ПОРАДА ДНЯ\n"
            f"[Найголовніше для Олега сьогодні виходячи з усіх транзитів і аспектів до його натальної карти. "
            f"Конкретно, без кліше. 2-3 речення.]\n\n"
            f"ПРАВИЛА:\n"
            f"- Перелічи ВСІ транзити і аспекти зі звіту — не пропускай жодного\n"
            f"- Завжди прив'язуй до НАТАЛЬНОЇ карти Олега (знак, дім, натальна планета)\n"
            f"- Тільки реальні дані зі звіту — не вигадуй аспекти\n"
            f"- БЕЗ вступів і прощань\n"
            f"- Мова: українська"
        )
        body = json.dumps({
            "contents": [{"parts": [{"text": prompt}]}],
