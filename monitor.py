...
def _get_astro_ai_analysis(astro_text: str, gemini_key: str, shift_hint: str = "") -> str:
    """AI аналіз астро-аспектів і домів натальної карти Олега."""
    if not astro_text or not gemini_key:
        return ""
    
    print(f"[astro_ai] generating analysis...", flush=True)
    
    # РОЗШИРЕНИЙ промпт: ФОКУС НА АСПЕКТАХ (квадратури, тригони, опозиції) і ДОМАХ
    prompt = f"""Ти експерт-астролог для Олега Новосадова (народження 22.09.1989 о 02:54, Львів, Україна).
Сьогодні Олег перебуває в Косіце, Словаччина.

АНАЛІЗУЙ ТІЛЬКИ НАСТУПНЕ (не загальні речи про здоров'я/роботу):
1️⃣ ОСНОВНІ АСПЕКТИ його натальної карти:
   - Мажорні аспекти (квадратури ☐, тригони △, опозиції ☍) між його планетами
   - Що означає кожен аспект: сильні місця, напруги, конфлікти, таланти
2️⃣ ДОМА (в яких домах його планети):
   - 1-й дім: Як він себе сприймає і як його сприймають інші
   - 10-й дім: Карєра, публічний образ, цілі
   - 7-й дім: Стосунки, партнерства
   - 8-й дім: Спільні ресурси, психологія, трансформація
   - 5-й дім: Творчість, романтика, ризик
3️⃣ ТРАНЗИТИ НА СЬОГОДНІ:
   - Які планети сьогодні аспектують його натальні планети
   - Короткий прогноз впливу на день

ФОРМАТ: 300-350 слів, структуровано за пунктами, БЕЗ хвастощів, прямо до суті, УКРАЇНСЬКОЮ.
