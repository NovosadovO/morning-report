...
def get_weather():
    ...
    # Сон — додаємо в погодний блок (зранку актуально)
    sleep_line = ""
    try:
        import sys as _sys
...
def _get_run_recommendation(weather_text):
    """Рекомендація бігти сьогодні — на основі погоди та останнього бігу."""
    import re as _re
    ...
    try:
        ...
        now_local_dt = now_utc + timedelta(hours=2)
        run_dates = []
        for line in content.split('\n'):
            if 'HKWorkoutActivityTypeRunning' not in line:
                continue
            m = _re.search(r'startDate="([^"]+)"', line)
            if m:
                s = m.group(1).strip()
                s = _re.sub(r' ([+-]\d{4})
...
, r'\1', s).replace(' ', 'T', 1)
        ...
        if run_dates:
            ...
            last_run_days = (now_utc - last_run).days
    except Exception as e:
        print(f"run recommendation error: {e}")

    # Погода для бігу
    bad_weather = any(x in weather_text.lower() for x in ["гроза", "сильний дощ", "сніг", "злива"])
    light_rain  = any(x in weather_text.lower() for x in ["дощ", "мряка"])

    msg = ""
    if last_run_days is None:
        if not bad_weather:
            msg = "🏃 Дані про пробіжки не знайдено — саме час вийти!"
    elif last_run_days == 0:
        msg = "🏃 Сьогодні вже бігав — молодець! 💪"
    elif last_run_days == 1:
        if bad_weather:
            msg = "🏃 Вчора бігав. Погода так собі — відпочинь або біжи під дощем 🌧"
        else:
            msg = "🏃 Вчора бігав — тіло відновлюється. Можна сьогодні!"
    elif last_run_days <= 3:
        if bad_weather:
            msg = f"🏃 Остання пробіжка {last_run_days} дні тому. Погода зла — але можеш! 💪"
        else:
            msg = f"🏃 {last_run_days} дні без бігу — гарний момент вийти сьогодні!"
    else:
        if bad_weather:
            msg = f"🏃 {last_run_days} днів без бігу... Погода не дуже, але ліньки гірше 😄"
        else:
            msg = f"🏃 <b>{last_run_days} днів без бігу!</b> Сьогодні — обов'язково! 💨"
