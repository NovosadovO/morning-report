...
def _get_shift_type_today():
    """
    Повертає тип зміни: 'early' | 'night' | 'day_off'
    Читає події з Google Calendar на сьогодні.
    """
    creds_json = os.environ.get("GOOGLE_CALENDAR_CREDENTIALS", "")
    if not creds_json:
        return "day_off"
    try:
        ...
        day_end   = now_local.replace(hour=23, minute=59, second=59, microsecond=0) - timedelta(hours=2)
        url = (
            f"https://www.googleapis.com/calendar/v3/calendars/{cal_id}/events"
            f"?timeMin={urllib.parse.quote(day_start.isoformat())}"
            f"&timeMax={urllib.parse.quote(day_end.isoformat())}"
        ...
        with urllib.request.urlopen(req, timeout=15) as r:
            events = json.loads(r.read()).get("items", [])
        titles = " ".join(ev.get("summary", "").lower() for ev in events)
        if "рання" in titles or "early" in titles:
            return "early"
        if "нічна" in titles or "ніч" in titles or "night" in titles:
            return "night"
        return "day_off"
    except Exception as e:
        print(f"_get_shift_type_today error: {e}")
        return "day_off"


def _send_meds_message(today, is_repeat=False):
    """Надсилає повідомлення про ліки з кнопками."""
    bot_token  = os.environ.get("TELEGRAM_TOKEN", "8374312425:AAHqrQCEqrgtVdl5Te5WhWblM2ESCnqhpfk")
    chat_id_tg = os.environ.get("TELEGRAM_CHAT_ID", "2100366814")
    repeat_str = "\n⚠️ <b>Ти ще не відмітив прийом!</b>" if is_repeat else ""
    text = (
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        "💊 <b>ARMOLOPID PLUS</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Час прийняти таблетку від холестерину!{repeat_str}\n\n"
...
def check_meds_reminder():
    ...
    - Рання зміна  → о 05:00, повтор о 07:00 якщо не відмітив
    - Нічна зміна  → о 17:10, повтор о 19:10 якщо не відмітив
    - Вихідний     → о 11:00, повтор о 13:00 якщо не відмітив
    Якщо вже натиснув ✅ або ❌ — більше не турбує.
    """
    now_local = datetime.now(timezone.utc) + timedelta(hours=2)
    h, m = now_local.hour, now_local.minute
    today = now_local.strftime("%Y-%m-%d")

    # Вікна спрацювання (h, m_from, m_to, slot_key, is_repeat)
    WINDOWS = [
        (5,  0,  9, "early_main",    False),  # рання — основне
        (7,  0,  9, "early_repeat",  True),   # рання — повтор
        (17, 10, 19, "night_main",   False),  # нічна — основне
        (19, 10, 19, "night_repeat", True),   # нічна — повтор
        (11,  0,  9, "off_main",     False),  # вихідний — основне
        (13,  0,  9, "off_repeat",   True),   # вихідний — повтор
    ]

    matched = None
    for wh, wm_from, wm_to, slot, repeat in WINDOWS:
        if h == wh and wm_from <= m <= wm_to:
            matched = (slot, repeat)
            break
    if not matched:
        return

    slot, is_repeat = matched

    state = load_json_file(MEDS_FILE, default={})

    # Якщо вже відреагував сьогодні (✅ або ❌) — не турбуємо
    if state.get(f"done_{today}"):
        return

    # Якщо вже надсилали цей конкретний слот
    if state.get(f"sent_{today}_{slot}"):
        return

    # Повтор надсилаємо тільки якщо основне вже було надіслано
    if is_repeat:
        # визначаємо пару основного слоту
        main_slot = slot.replace("_repeat", "_main")
        if not state.get(f"sent_{today}_{main_slot}"):
            return  # основне ще не надсилали — пропускаємо повтор

    # Перевіряємо тип зміни — чи відповідає поточний слот
    shift = _get_shift_type_today()
    if "early" in slot and shift != "early":
        return
    if "night" in slot and shift != "night":
        return
    if "off" in slot and shift != "day_off":
        return

    try:
        _send_meds_message(today, is_repeat=is_repeat)
        print(f"Meds reminder sent: slot={slot} shift={shift} repeat={is_repeat}")
        state[f"sent_{today}_{slot}"] = True
