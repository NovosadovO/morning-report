...
_GEM_MODELS = ["gemini-2.5-flash-lite", "gemini-2.5-flash", "gemini-3.5-flash-lite"]  # 23.09: платний ключ поповнено (auto-reload ВИМКНЕНО, жорсткий budget-cap). flash-lite першим — набагато дешевший за flash (менше "thinking"-токенів), flash як резерв для якості/при 429
...
def _gem_post(url, body_bytes, timeout=90, tag="gem", max_retries=3):
    ...
    перемикаємось на gemini-2.5-flash (інший quota-pool), потім gemini-3.5-flash-lite.
    (20.09: gemini-2.5-flash-lite видалено Google — 404 "no longer available
    to new users". Замінено на gemini-3.5-flash-lite, і додано switch-on-404,
    щоб наступна деприкація моделі так само не вбивала AI-блок мовчки.)
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if e.code in (500, 503) and attempt < max_retries - 1:
                    ...
                    _t.sleep(8)
                    continue
                # 404 = модель видалена/недоступна (Google деприкейтить моделі
                # без попередження — так сталось з gemini-2.5-flash-lite 20.09).
                # Раніше це падало в `raise` і виходило з ФУНКЦІЇ одразу, не
                # даючи шансу наступній моделі зі списку — тому AI-блок (напр.
                # astro_ai) миттєво йшов у local fallback, хоч інші моделі були
                # живі. Тепер — як вичерпана модель, одразу switch, без ретраїв
                # на тій самій (сенсу нема, вона просто не існує).
                if e.code == 404:
                    print(f"[{tag}] 404 on {_model} (модель видалена/недоступна) "
                          f"— switch до наступної", flush=True)
                    _exhausted_429 = True
                    break
                raise
            except Exception as e:
...
def _gemini_email_analysis(full_text: str, health_context: str = "") -> dict:
    """Аналізує лист через Gemini: детальний переказ + думка + автовиявлення дії
    (треба щось зробити: оплатити/записати в календар/купити тощо) — АІ сам
    пропонує нагадування ПРОАКТИВНО, без натискання окремої кнопки '📅 В календар'."""
    import re as _re
    api_key = os.environ.get("GEMINI_API_KEY", "")
    ...
    prompt = (
        ...
        "- 'calendar' якщо в листі є КОНКРЕТНА дія з дедлайном/датою (оплатити рахунок/страховку, "
        "з'явитись на зустріч, подати документи, продовжити підписку, тощо) — навіть якщо точної дати "
        "немає, але зрозуміло що треба зробити найближчим часом.\n"
        "- 'shopping' якщо лист явно згадує щось конкретне що треба КУПИТИ (товар, продукт, засіб).\n"
        "- 'none' якщо нема жодної конкретної дії (просто інформація, реклама, новини).\n"
        "action_title: коротка назва дії (напр. 'Оплатити страховку авто').\n"
...
def detect_actionable_item(text: str) -> dict:
    ...
    prompt = (
        ...
        "БУДЬ СУВОРИМ — 'none' якщо не абсолютно очевидно що потрібна дія. "
        "КРИТИЧНО ВАЖЛИВО: НЕ ВИГАДУЙ дій, дат чи товарів яких немає в тексті буквально — "
        "тільки те що РЕАЛЬНО написано. Якщо не впевнений — завжди 'none'. Краще пропустити "
        "реальну дію, ніж вигадати неіснуючу.\n"
...
def _build_report_header(now_local, slot_key, cal_events_raw):
    ...
    _vibes = {
        "early_morning": [
            ...
            "Дисципліна бігуна — щоденний старт 05:00 → 75кг ближче 🏃",
        ],
        "morning": [
            "Ранок — перевірив BTC, поїв, побіг. Це і є фінансова свобода 🚀",
            "Кожен кілометр бігу — крок до 75кг і нової версії себе 🏃💰",
...
def _get_astro_ai_analysis(astro_text: str, gemini_key: str, shift_hint: str = "") -> str:
    """AI аналіз НАТАЛЬНОЇ КАРТИ + ТРАНЗИТНИХ АСПЕКТІВ на дану годину."""
    ...
    # Отримуємо ТРАНЗИТНІ АСПЕКТИ (поточні планети відносно натальних)
    transit_data = ""
    try:
        import astro as _astro_mod
        transit_text = _astro_mod.get_natal_transits_short(max_aspects=12)
        if transit_text:
            # Видаляємо HTML теги для простоти
            import re as _re_html
            transit_data = _re_html.sub(r'<[^>]+>', '', transit_text)
    except Exception as e:
        print(f"[astro_ai] WARNING: couldn't get transits: {e}", flush=True)
        transit_data = ""

    # ТРАНЗИТИ ПО ДОМАХ + АСПЕКТИ З ДОМАМИ (запит Олега: більше опису)
    houses_block = ""
    try:
        import astro_houses as _ah
        houses_block = _ah.houses_facts(14) or ""
        print(f"[astro_ai] houses_facts: {len(houses_block)} chars", flush=True)
    except Exception as e:
        print(f"[astro_ai] WARNING: houses_facts failed: {e}", flush=True)
        houses_block = ""
    ...
    prompt = f"""Ти астролог. Аналізуй ТІЛЬКИ ПОТОЧНІ ТРАНЗИТИ для Олега Новосадова.
ДН: 22.09.1989, 02:52, Львів | СЬОГОДНІ: {today_str}, Косіце, Словаччина

━━━━━━━━━━━━━━━━━━ НАТАЛЬНА КАРТА (для довідки) ━━━━━━━━━━━━━━━━━━
Асцендент (ASC) 1°02' ЛЕВ (I дім) — ДУЖЕ ВАЖЛИВО для будь-яких транзитів у Лева!
☉ 28°55' Діва III | ☽ 27°11' Близнюки XI | ☿ Rx 5°2' Терези III
♀ 10°56' Скорпіон IV | ♂ 1°30' Терези III | ♃ 8°42' Рак XII
♄ 7°23' Козеріг VI | ♅ 1°23' Козеріг VI | ♆ 9°36' Козеріг VI
♇ 13°22' Скорпіон IV | ☊ 25°20' Водолій VIII
...
━━━━━━━━━━━━━━━━━━ ТРАНЗИТИ ПО ДОМАХ + АСПЕКТИ З ДОМАМИ ━━━━━━━━━━━━━━━━━━
{houses_block if houses_block else "(доми недоступні — не вигадуй номери домів)"}

━━━━━━━━━━━━━━━━━━ КОНТЕКСТ ━━━━━━━━━━━━━━━━━━
{shift_hint[:200] if shift_hint else "вільний день"}

━━━━━━━━━━━━━━━━━━ ЗАВДАННЯ ━━━━━━━━━━━━━━━━━━
Напиши 800-1000 слів УКРАЇНСЬКОЮ, розгорнуто і детально. ТІЛЬКИ про СЬОГОДНІШНІЙ ДЕНЬ — НЕ описуй статичні натальні аспекти!

ОБОВ'ЯЗКОВО (прямий запит Олега — він хоче БІЛЬШЕ опису):
• ПО ДОМАХ: для кожної значної транзитної планети скажи, у якому НАТАЛЬНОМУ ДОМІ вона зараз і що це означає для конкретної сфери життя Олега (робота/зміни, гроші, дім, здоров'я, стосунки, кар'єра, друзі, втома) — 2-4 речення на планету, не один рядок;
• ПО АСПЕКТАХ З ДОМАМИ: для кожного аспекту назви ОБИДВІ сфери, які він зчіпляє (дім транзитної планети ↔ дім натальної), і опиши, як саме вони взаємодіють, наростає тема чи слабне (орб + сходиться/розходиться), і що з цим робити;
• окремо виділи те, що СХОДИТЬСЯ і має точну дату — це те, до чого готуватись;
• зв'яжи астро з його реальним життям із зрізу даних (зміни, сон, вага, гроші, дедлайни) — не абстрактно.

КРИТИЧНО ВАЖЛИВО — ПРІОРИТЕТ ПОДІЙ:
1. НАЙВИЩИЙ пріоритет: якщо в транзитах є "увійшов у знак твого Асценденту" (ingress) — це РІДКІСНА подія (триває роками), ОБОВ'ЯЗКОВО опиши її ПЕРШОЮ і найдетальніше — що означає для особистості, зовнішності, нового циклу.
2. Високий пріоритет: точні аспекти (орб <1.5°) до Асценденту, Сонця, Марса — вони формують ідентичність і дію.
3. Секстилі (⚹) і трини (△) від Юпітера — це ВІКНО МОЖЛИВОСТЕЙ, не пропускай їх у ТОП-3.
4. НЕ ігноруй жоден аспект з наданого списку транзитів — усі вони релевантні, обери найважливіші для ТОП-3, але згадай коротко й інші.

Структура:
🌍 ТРАНЗИТИ СЬОГОДНІ — що рухається, які аспекти формуються до натальних планет (і до Асценденту!)
  → Для кожного транзиту: [Транзитна планета] [аспект] [Натальна планета/позиція] → що це означає конкретно
🔴/🟢/🔵 Напруга / Гармонія / Баланс — 1-2 реченя на кожен активний транзит
⚡ ТОП-3 ТЕМИ ДНЯ — найвпливовіші транзити сьогодні, враховуючи пріоритети вище (1-2 рядки кожна)
🎯 РЕКОМЕНДАЦІЇ — 3 конкретні дії з урахуванням астро-енергії (НЕ загальні слова)

ПРАВИЛА:
- НЕ повторюй статичні натальні аспекти між натальними планетами — нас цікавлять ТІЛЬКИ транзит→натал аспекти СЬОГОДНІ
...
def _ai_personal_message(situation: str, context: dict = None, max_tokens: int = 200) -> str:
    ...
    try:
        ...
        if wd:
            ...
            ctx_parts.append(f"Вага: {wd[last_key]} кг (ціль 75 кг, залишилось {wd[last_key]-75:.1f} кг)")
    except: pass
...
def check_smart_notifications():
    ...
    try:
        ...
        # ── 7. ПРОГРЕС ДО 75 КГ (щосереди о 12:00) ───────────────────────
            ...
            try:
                ...
                try:
                    ...
                    if wdata:
                        ...
                        weight_ctx = f"Вага: {wdata[last_key]} кг (ціль 75, залишилось {wdata[last_key]-75:.1f} кг)."
                except Exception:
                    pass
    ...
    try:
        ...
        if gemini_key:
            try:
                ...
                w_info = f", вага {w_end} кг (ціль 75)" if w_end else ""
                prompt = (
...
def check_health_weekly_tracker():
    ...
    if weight_vals:
        ...
        ctx_lines.append(f"Вага: початок {w_start}кг → кінець {w_end}кг ({trend}{abs(w_diff)}кг), середня {w_avg}кг, ціль 75кг (залишилось {round(w_end-75,1)}кг)")
    else:
...
def _get_daily_recommendations(context: dict, gemini_key: str = None) -> str:
    """
    AI рекомендація дня: обсяг під зміст (від 3 щільних рядків до ~400 слів)
    ...
    try:
        ...
        prompt = (
            ...
            f"🌙 АСТРО: {astro_ctx}\n\n"
            f"ТВОЯ ЗАДАЧА:\n"
            f"Напиши аналіз дня для Олега українською. Обсяг — під зміст: якщо є одна "
            f"справжня думка, дай 3-5 щільних рядків; розгортайся тільки коли є про що. "
            f"Кожен рядок мусить нести цифру, висновок або дію:\n"
    ...
    try:
        ...
        try:
            ...
            if _wd_th:
                ...
                _health_parts.append(f"Вага: {_wd_th[_lk]} кг, ціль 75 кг{_trend}")
            if steps_val > 0:
        ...
        for _attempt_b in range(1):  # 1 прохід — _gem_post всередині ретраїть на 429
            try:
                ...
                _brief_prompt = (
                    ...
                    f"(1) фінансова незалежність через крипто і InterFin, (2) схуднення до 75 кг, (3) нова робота в інвестиціях. "
