    ...
    def _do_request(payload_bytes):
        ...
        try:
            ...
            if _q_g2.blocked("msg"):
                print("[quiet] 🌙 сон: _do_request пропущено", flush=True)
                return None
        except Exception:
            pass
        req = urllib.request.Request(
...
def check_proactive():
    ...
    # ── Обідній check-in (12:00–13:00, тільки вдома/вільний день) ───────────
        ...
        prompt = (
            ...
            f"Спитай як пройшов ранок, чи поїв нормально (ціль 75 кг, дієта 16:8). "
...
Профіль: заводський робітник Minebea Mitsumi (змінний графік), цілі — схуднути до 75 кг, бігати 40 км/міс, фінансова незалежність через крипто+ETF.
