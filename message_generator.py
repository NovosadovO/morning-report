...
def _generate_message(trigger_type: str, trigger_data, location: str, idle_hours: float) -> str:
    ...
    if trigger_type == "crypto_move" and isinstance(trigger_data, dict):
        ...
        trigger_extra = (
            ...
            "для цілі 75 кг). Використай вагу з живих даних. Без загальних фраз типу 'їж здорову їжу' — "
        ...
        _mc_themes = [
            ...
            ("ціль_вага", "Запитай як просувається ціль схуднення до 75кг — чи тримається плану сьогодні/цього тижня, що заважає якщо є труднощі. Одне конкретне питання."),
    ...
    if trigger_type in SHORT_TRIGGERS:
        ...
        max_tokens = 3500
        length_instruction = ("Напиши ЖИВЕ, ОСОБИСТЕ повідомлення УКРАЇНСЬКОЮ. "
                             "Обсяг — під зміст, а не під норму: якщо є одна справжня "
                             "думка, дай 3-5 рядків; якщо справді є про що — до 500 слів. "
                             "Не розтягуй заради обсягу: розведена вода гірша за короткий "
                             "щільний текст.")
        structure_instruction = ("Структура вільна — вибери її під матеріал. Обов'язково: "
                                "сильний перший рядок → аналіз ЛИШЕ тих блоків, де є "
                                "реальні дані (порожні блоки не згадуй взагалі) → 1-3 "
                                "конкретні дії. Кожен рядок мусить нести цифру, висновок "
                                "або дію — інакше видали його.")

    # ── Зворотний зв'язок: що Олег насправді натискав/відповідав ──────────────
    fb_ctx = ""
    try:
        import feedback_ctx as _fb_mg
        fb_ctx = _fb_mg.build(days=7) or ""
    except Exception as _e_fb_mg:
        print(f"[MG] feedback_ctx error: {_e_fb_mg}", flush=True)

    # ── Prompt ────────────────────────────────────────────────────────────────
    prompt = f"""Ти персональний AI-асистент Олега Новосадова (36р, Кошіце SK, Minebea Mitsumi, крипто-інвестор, бігун, ціль: фінансова незалежність + схуднення до 75 кг).
...
8. ЯКЩО є блок "ЗВОРОТНИЙ ЗВ'ЯЗОК ОЛЕГА" — врахуй його: згадай що він позначив зробленим, не піднімай теми які він приховав, продовж думку з його нотаток. Це показує що ти пам'ятаєш його реакції.
