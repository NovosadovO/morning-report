...
def check_motivation_7am():
    """
    О 7:00 — персоналізоване мотиваційне повідомлення від AI на новий день.
    """
    now = _now()
    if not (now.hour == 7 and now.minute <= 10):
        return
    ...
    today_key = now.strftime("%Y-%m-%d")
    state = _load_state("assistant_motivation_7am.json", {})
    if state.get(today_key):
        return

    weekday_ua = ["Понеділок", "Вівторок", "Середа", "Четвер", "П'ятниця", "Субота", "Неділя"]
    day_name = weekday_ua[now.weekday()]
    date_str = now.strftime("%d.%m.%Y")

    prompt = f"""Ти особистий AI-коуч для Олега. Сьогодні {day_name}, {date_str}.

Про Олега:
- Живе в Кошіце, Словаччина
- Інтереси: крипто (BTC, ETH, AVAX, ONDO), інвестиції, біг, здоровий спосіб життя
- Ціль: фінансова незалежність, схуднення (зараз ~83-84 кг), нова робота у сфері інвестицій
- Працює змінний графік на заводі Minebea Mitsumi

Напиши коротке мотиваційне повідомлення для Олега на початок дня. Вимоги:
- 4-6 речень, конкретно і по суті
- Один практичний фокус на день (інвестиції, здоров'я або навчання)
- Надихаючий тон, без банальщини
- Мова: українська
- Без зайвих привітань типу "Привіт!" або "Доброго ранку, Олеже!"
- Починай одразу з суті або короткого інсайту"""

    text = _gemini(prompt, max_tokens=300)
    if not text:
        text = f"Новий день — нові можливості. Зроби сьогодні один крок до фінансової незалежності: перечитай один урок про інвестиції або провери свій крипто-портфель. Маленькі кроки щодня = великий результат через рік."

    state[today_key] = True
    _save_state("assistant_motivation_7am.json", state)

    _send(f"🌅 <b>{day_name}, {date_str}</b>\n\n{_esc(text)}")
    print(f"[assistant] motivation_7am sent")
