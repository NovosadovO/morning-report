...
_REGISTRIES = (
    ...
    # health.json ВИДАЛЕНО з реєстру (25.09 фікс): це мертвий локальний
    # файл (не оновлювався з 2026-05-11), і _reg() дампив його СИРИМ JSON
    # напряму, без мерджу з канонічним qwatch_data.json — AI бачив
    # 4-місячної давності сон/кроки як окремий блок поряд із актуальними
    # даними з healthai.facts_block()/allctx (які й так інжектяться в
    # промпти), що створювало суперечливі цифри. Актуальні health-дані
    # AI отримує через healthai.analytics()/facts_block() (канонічний
    # storage.load_health()), тут дублювати їх сирим мертвим файлом не треба.
